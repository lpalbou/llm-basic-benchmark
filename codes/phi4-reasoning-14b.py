#!/usr/bin/env python3
"""
A simulation where a ball bounces off a rotating triangle.
The triangle rotates continuously while the ball moves at constant speed.
If the ball crosses an edge, its velocity is reflected according to the edge’s inward normal.
"""
...
def main():
    ...
    while running:
        ...
        pygame.draw.polygon(screen, WHITE, [v.x for v in triangle_vertices], 2)
        
        # Optionally, draw the triangle’s center
        pygame.draw.circle(screen, RED, (int(center.x), int(center.y)), 4)
